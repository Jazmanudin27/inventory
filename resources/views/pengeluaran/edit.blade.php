...
@section('titlepage', 'Edit Data Pengeluaran')
@section('admin')
    <div class="container">
        <div class="element-heading">
            <h6 style="text-align: center">Input Data Pengeluaran</h6>
        </div>
    </div>
    <div class="container">
        <form method="POST" action="{{ route('pengeluaran.update', $pengeluaran->nobukti) }}" autocomplete="off">
            @csrf
            <div class="card">
                <div class="card-body">
                    <div class="form-group">
                        <label class="form-label">No Bukti</label>
                        <input class="form-control form-control-sm" value="{{ $pengeluaran->nobukti }}" type="text"
                            id="nobukti" name="nobukti" placeholder="Nobukti">
                    </div>
                    <div class="form-group">
                        <label class="form-label">Tanggal</label>
                        <input class="form-control form-control-sm" value="{{ $pengeluaran->tanggal }}" type="date"
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-select form-select-sm" id="jenis_pengeluaran" name="jenis_pengeluaran">
                            ...
                            <option {{ $pengeluaran->jenis_pengeluaran == 'Penjualan' ? 'selected' : '' }}
                                value="Penjualan">
                                Penjualan
                            </option>
    ...
    <script type="text/javascript">
        $(document).ready(function($) {
            ...
            function detailBarang() {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ route('pengeluaran.detailBarang') }}",
                    data: {
                        _token: "{{ csrf_token() }}",
                        nobukti: nobukti,
                    },
            ...
            $('#tambah').click(function(e) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ route('pengeluaran.StoreDetail') }}",
                    data: {
                        _token: "{{ csrf_token() }}",
                        nobukti: nobukti,
